...
#esta es una prediccion con la muestra total, considero que queda mejor porque el peak_age se observa alrededor de 55 aÒos
...
ggplot(geih_ef, aes(x=age, y=predict(ols1f))) + 
  geom_point(col = "red" , size = 0.5) #aqui vemos que la peak_age se observa hacia los 38 aÒos
...
ggplot(geih_em, aes(x=age, y=predict(ols1m))) + 
  geom_point(col = "red" , size = 0.5) #aqui vemos que la peak_age se observa hacia los 48 aÒos!
...
##ahora vamos a calcular el valor de esos peak_age especÌficamente
#Recordar: 
